...
_Generated at 2026-05-07T08:26:05Z from workflow-spec.yaml_
...
- `stride` role=`threat_analysis` template=`fan-out` gate=`auto_approval` loop=`disabled` outputs=`outputs/stride-audit/threat_list.json`
...
- `report` role=`report_assembly` template=`loop` gate=`auto_approval` loop=`ralph` outputs=`outputs/stride-audit/stride-audit-report.html`
...
- ALWAYS:
  ...
  - Every threat must be classified as [VULN]/[DESIGN]/[HARDENING]/[FP]/[OOS]
