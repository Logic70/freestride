...
THREAT_DFD_MAP = {
    ...
    'T-020':['P1'],
    # THREAT-XXX format (canonical)
    'THREAT-001':['P5','P3'],'THREAT-002':['P2','P12'],'THREAT-003':['P2','P12'],
    'THREAT-004':['P3'],'THREAT-005':['P1','P16'],'THREAT-006':['P16','EE3'],
    'THREAT-007':['P1'],'THREAT-008':['P4'],'THREAT-009':['P15','P1'],
    'THREAT-010':['P8'],'THREAT-011':['P6'],'THREAT-012':['P15','EE1'],
    'THREAT-013':['P1','EE1'],'THREAT-014':['P8'],'THREAT-015':['P2','P12'],
    'THREAT-016':['P2'],'THREAT-017':['P1','EE5'],'THREAT-018':['P12','DS1','DS2'],
    'THREAT-019':['P12','DS1','DS2'],'THREAT-020':['P1'],
    'THREAT-021':['P16'],'THREAT-022':['P5'],'THREAT-023':['P16','EE3'],
    'THREAT-024':['P1','P8'],
}

# ============================================================
# 元素短名称（用于 dfd_index name 字段）
# ============================================================
ELEMENT_NAME = {
    'EE1':'Normal App Process (Client App)',
    'EE2':'Remote Peer Device',
    'EE3':'HUKS (Hardware Unique Key Store)',
    'EE5':'DSoftBus Transport',
    'P1':'System Ability Entry (SA 4701)',
    'P2':'Group Manager',
    'P3':'Group Auth Manager',
    'P4':'Identity Service',
    'P5':'Protocol Engine',
    'P6':'Key Agreement SDK',
    'P8':'Session Manager',
    'P12':'Data Persistence Layer',
    'P15':'Framework Layer',
    'P16':'Crypto Adapter (HUKS/mbedTLS)',
    'DS1':'Group Membership Database',
    'DS2':'Credential Database',
    'DS3':'Session Cache (In-Memory)',
}

# ============================================================
# 元素描述（用于 tooltip/详细说明）
# ============================================================
ELEMENT_DESC = {
...
def generate_dfd_index(threats, dfd_data):
    ...
    idx = {}
    # First pass: create entries from dfd.yaml elements (full IDs like PROC_001, EXT_001)
    ...
    # Second pass: add simplified ID entries for SVG rendering (P1, P2, EE1, DS1)
    simplified_type_map = {}
    for eid in NODE_POSITIONS:
        if eid.startswith('EE'):
            simplified_type_map[eid] = 'external_entities'
        elif eid.startswith('DS'):
            simplified_type_map[eid] = 'data_stores'
        else:
            simplified_type_map[eid] = 'processes'

    for eid, cat in simplified_type_map.items():
        if eid not in idx.get(cat, {}):
            name = ELEMENT_NAME.get(eid)
            if name is None:
                print(f'[WARN] dfd_data: simplified ID "{eid}" missing from ELEMENT_NAME — using raw eid as fallback')
                name = eid
            idx[cat][eid] = {
                'name': name,
                'description': ELEMENT_DESC.get(eid, ''),
                'type': cat, 'threats': [], 'threat_count': 0
            }

    # Map threats to simplified IDs (from THREAT_DFD_MAP) and also to full IDs
    for t in threats:
        mapped_ids = THREAT_DFD_MAP.get(t['id'], [])
...
# ============================================================
# 模块自检：NODE_POSITIONS 的每个 key 必须在 ELEMENT_NAME 和 ELEMENT_DESC 中都有对应
# ============================================================
_missing_name = set(NODE_POSITIONS) - set(ELEMENT_NAME)
_missing_desc = set(NODE_POSITIONS) - set(ELEMENT_DESC)
assert not _missing_name, f'NODE_POSITIONS keys missing from ELEMENT_NAME: {_missing_name}'
assert not _missing_desc, f'NODE_POSITIONS keys missing from ELEMENT_DESC: {_missing_desc}'
