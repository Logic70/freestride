...
**流程**: parse → dfd → stride → verify → validation → poc → result-auditor(pre) → report → ui-verify → [diff]

**v0.5+ 新增**:
- 浏览器 UI 验证 (ui-verify): headless Chromium 验证 DFD 交互合约、JS 错误、section anchors（MCP Playwright + Python 烟雾测试）
